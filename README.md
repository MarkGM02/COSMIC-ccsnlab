...
3. **Rerun scripts for COSMIC**, which incorporate the detailed **common-envelope evolution (CEE)** prescription of  
   [Klencki et al. (2021)](https://ui.adsabs.harvard.edu/abs/2021A%26A...645A..54K/abstract).  
4. **Our sampling and population evolution scripts**, allowing users to recreate our populations if so desired.
...
Here, all columns marked with an asterisk (`_*`) appear twice — once for the **first** and once for the **second** supernova in the binary.
...
One of the plotting scripts uses the external package  
