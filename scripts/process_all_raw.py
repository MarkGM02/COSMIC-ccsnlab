"""Extensive version of process_raw.py that loads in ZAMS and SN data across a grid of parameters and metallicities, set to work with
...
def load_zams_and_sn(qcflag, grid_type, params, metallicities, rerun_prescription=None):
    """Load ZAMS and SN data from COSMIC simulations across a grid of parameters and metallicities. Use without modification requires
    the directory structure to match that below -- which selects the right population from the input parameters. IIn classification will be
    missed if bcm dataframes with the densest setting in the last kyr before the SN are not present.
    """
    final_data = []
...
def create_static_data(job, qcflag=5):
    """Create processed dataframes for various parameter grids. Requires the raw data to be present in the expected directory structure.
    Saves the processed dataframes to processed_data/qcflag_{qcflag}_*.h5"""
